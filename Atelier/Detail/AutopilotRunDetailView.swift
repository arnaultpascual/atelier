...
struct AutopilotRunDetailView: View {
    ...
    // MARK: Feature deliverable (auto-generated by the final synthesis pass)

    @ViewBuilder
    private var deliverableSection: some View {
        if let path = record.deliverablePath, FileManager.default.fileExists(atPath: path) {
            let url = URL(fileURLWithPath: path)
            VStack(alignment: .leading, spacing: 6) {
                HStack(spacing: 6) {
                    Image(systemName: "doc.text.fill").foregroundStyle(Color.atelierAccent).font(.system(size: 11))
                    Text("FEATURE DELIVERABLE")
                        .font(AtelierFont.eyebrow.weight(.semibold)).foregroundStyle(Color.atelierInk)
                    Spacer()
                ...
                Text(url.lastPathComponent)
                    .font(AtelierFont.captionMono).foregroundStyle(Color.atelierInkSecondary)
                    .lineLimit(1).truncationMode(.middle)
                Text("Technique · quality & coverage · manual tests — written at the project root.")
                    .font(AtelierFont.eyebrow).foregroundStyle(Color.atelierInkSecondary)
            }
            .padding(12)
            .frame(maxWidth: .infinity, alignment: .leading)
            .background(Color.atelierAccentSoft.opacity(0.25), in: RoundedRectangle(cornerRadius: AtelierCorner.card))
            .overlay(RoundedRectangle(cornerRadius: AtelierCorner.card).stroke(Color.atelierAccent.opacity(0.3), lineWidth: 1))
        }
    }
