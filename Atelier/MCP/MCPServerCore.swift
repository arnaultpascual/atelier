    ...
    var tools: [ToolDef] {
        [
            ...
            ToolDef(name: "task_update_status",
                description: "Set a task's status atomically (To Do / In Progress / Review / Done / Blocked). Keeps the backlog .md and DB in sync.",
                inputSchema: Self.schema([
                    "taskId": Self.strProp("Task id; defaults to the current task."),
                    "status": Self.strProp("One of: To Do, In Progress, Review, Done, Blocked."),
                ], required: ["status"])),
            ...
            ToolDef(name: "wave_mark_done",
                description: "Signal the current dependency wave is complete for the feature.",
            ...
            ToolDef(name: "coverage_get",
                description: "Get current test coverage vs the soft 90% target for this feature: { current, target, gap, mode }.",
                inputSchema: Self.schema([:], required: [])),
            ...
            ToolDef(name: "test_report_run",
                description: "Report a structured test run (feeds the feature's test dossier).",
                inputSchema: Self.schema([
